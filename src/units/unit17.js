import good from '../img/good.jpg';
import be from '../img/be.jpg';
import beaten from '../img/beaten.jpg';
import becomeBecame from '../img/becomeBecame.jpg';
import begun from '../img/begun.jpg';
import breakA from '../img/breakA.jpg';
import bringBrought from '../img/bringBrought.jpg';
import built from '../img/built.jpg';
import buyBought from '../img/buyBought.jpg';
import catchA from '../img/catchA.jpg';
import choose from '../img/choose.jpg';
import comeCame from '../img/comeCame.jpg';
import cost from '../img/cost.jpg';
import cut from '../img/cut.jpg';
import dug from '../img/dug.jpg';
import done from '../img/done.jpg';
import drawn from '../img/drawn.jpg';
import dreamt from '../img/dreamt.jpg';
import drunk from '../img/drunk.jpg';
import learnToDrive from '../img/learnToDrive.jpg';
import eaten from '../img/eaten.jpg';
import fallFell from '../img/fallFell.jpg';
import feel from '../img/feel.jpg';
import fought from '../img/fought.jpg';
import findFound from '../img/findFound.jpg';
import flyFlew from '../img/flyFlew.jpg';
import forget from '../img/forget.jpg';
import getGot from '../img/getGot.jpg';
import giveGave from '../img/giveGave.jpg';
import gone from '../img/gone.jpg';
import grown from '../img/grown.jpg';
import had from '../img/had.jpg';
import hear from '../img/hear.jpg';
import hidden from '../img/hidden.jpg';
import hit from '../img/hit.jpg';
import hurt from '../img/hurt.jpg';
import kept from '../img/kept.jpg';
import know from '../img/know.jpg';
import learn from '../img/learn.jpg';
import leave from '../img/leave.jpg';
import loseLost from '../img/loseLost.jpg';
import make from '../img/make.jpg';
import meant from '../img/meant.jpg';
import meet from '../img/meet.jpg';
import pay from '../img/pay.jpg';
import put from '../img/put.jpg';
import readRead from '../img/readRead.jpg';
import ring from '../img/ring.jpg';
import runRan from '../img/runRan.jpg';
import say from '../img/say.jpg';
import seeSaw from '../img/seeSaw.jpg';
import sellSold from '../img/sellSold.jpg';
import sendLetters from '../img/sendLetters.jpg';
import show from '../img/show.jpg';
import shut from '../img/shut.jpg';
import singSang from '../img/singSang.jpg';
import sit from '../img/sit.jpg';
import sleepSlept from '../img/sleepSlept.jpg';
import speak from '../img/speak.jpg';
import spendSpent from '../img/spendSpent.jpg';
import stand from '../img/stand.jpg';
import swimming from '../img/swimming.jpg';
import take from '../img/take.jpg';
import teachTaught from '../img/teachTaught.jpg';
import tell from '../img/tell.jpg';
import thinkThought from '../img/thinkThought.jpg';
import thrown from '../img/thrown.jpg';
import understandUnderstood from '../img/understandUnderstood.jpg';
import wake from '../img/wake.jpg';
import wearWore from '../img/wearWore.jpg';
import win from '../img/win.jpg';
import writeWrote from '../img/writeWrote.jpg';



const unit17 = [
    {
        word: `Три формы глагола "БЫТЬ"`,
        translate: 'be - was/were - been', 
        url: be
    },
    {
        word: `Три формы глагола "ПОБИТЬ"`,
        translate: 'beat - beat - beaten', 
        url: beaten
    },
    {
        word: `Три формы глагола "СТАНОВИТЬСЯ"`,
        translate: 'become - became - become', 
        url: becomeBecame
    },
    {
        word: `Три формы глагола "НАЧАТЬ"`,
        translate: 'begin - began - begun', 
        url: begun
    },
    {
        word: `Три формы глагола "ЛОМАТЬ"`,
        translate: 'break - broke - broken', 
        url: breakA
    },
    {
        word: `Три формы глагола "ПРИНОСИТЬ"`,
        translate: 'bring - brought - brought', 
        url: bringBrought
    },
    {
        word: `Три формы глагола "СТРОИТЬ"`,
        translate: 'build - built - built', 
        url: built
    },
    {
        word: `Три формы глагола "ПОКУПАТЬ"`,
        translate: 'buy - bought - bought', 
        url: buyBought
    },
    {
        word: `Три формы глагола "ЛОВИТЬ"`,
        translate: 'catch - caught - caught', 
        url: catchA
    },
    {
        word: `Три формы глагола "ВЫБИРАТЬ"`,
        translate: 'choose - chose - choosen', 
        url: choose
    },
    {
        word: `Три формы глагола "ПРИХОДИТЬ"`,
        translate: 'come - came - come', 
        url: comeCame
    },
    {
        word: `Три формы глагола "СТОИТЬ"`,
        translate: 'cost - cost - cost', 
        url: cost
    },
    {
        word: `Три формы глагола "РЕЗАТЬ"`,
        translate: 'cut - cut - cut', 
        url: cut
    },
    {
        word: `Три формы глагола "КОПАТЬ"`,
        translate: 'dig - dug - dug', 
        url: dug
    },
    {
        word: `Три формы глагола "ДЕЛАТЬ"`,
        translate: 'do - did - done', 
        url: done
    },
    {
        word: `Три формы глагола "РИСОВАТЬ"`,
        translate: 'draw - drew - drawn', 
        url: drawn
    },
    {
        word: `Три формы глагола "МЕЧТАТЬ"`,
        translate: 'dream - dreamt - dreamt', 
        url: dreamt
    },
    {
        word: `Три формы глагола "ПИТЬ"`,
        translate: 'drink - drank - drunk', 
        url: drunk
    },
    {
        word: `Три формы глагола "ВОДИТЬ (авто)"`,
        translate: 'drive - drove - driven', 
        url: learnToDrive
    },
    {
        word: `Три формы глагола "ЕСТЬ"`,
        translate: 'eat - ate - eaten', 
        url: eaten
    },
    {
        word: `Три формы глагола "ПАДАТЬ"`,
        translate: 'fall - fell - fallen', 
        url: fallFell
    },
    {
        word: `Три формы глагола "ЧУВСТВОВАТЬ"`,
        translate: 'feel - felt - felt', 
        url: feel
    },
    {
        word: `Три формы глагола "ДРАТЬСЯ"`,
        translate: 'fight - fought - fought', 
        url: fought
    },
    {
        word: `Три формы глагола "НАХОДИТЬ"`,
        translate: 'find - found - found', 
        url: findFound
    },
    {
        word: `Три формы глагола "ЛЕТАТЬ"`,
        translate: 'fly - flew - flown', 
        url: flyFlew
    },
    {
        word: `Три формы глагола "ЗАБЫВАТЬ"`,
        translate: 'forget - forgot - forgotten', 
        url: forget
    },
    {
        word: `Три формы глагола "ПОЛУЧАТЬ"`,
        translate: 'get - got - got', 
        url: getGot
    },
    {
        word: `Три формы глагола "ДАВАТЬ"`,
        translate: 'give - gave - given', 
        url: giveGave
    },
    {
        word: `Три формы глагола "ИДТИ"`,
        translate: 'go - went - gone', 
        url: gone
    },
    {
        word: `Три формы глагола "РАСТИ"`,
        translate: 'grow - grew - grown', 
        url: grown
    },
    {
        word: `Три формы глагола "ИМЕТЬ"`,
        translate: 'have - had - had', 
        url: had
    },
    {
        word: `Три формы глагола "СЛЫШАТЬ"`,
        translate: 'hear - heard - heard', 
        url: hear
    },
    {
        word: `Три формы глагола "ПРЯТАТЬ"`,
        translate: 'hide - hid - hidden', 
        url: hidden
    },
    {
        word: `Три формы глагола "УДАРИТЬ"`,
        translate: 'hit - hit - hit', 
        url: hit
    },
    {
        word: `Три формы глагола "РАНИТЬ"`,
        translate: 'hurt - hurt - hurt', 
        url: hurt
    },
    {
        word: `Три формы глагола "Держать (СОХРАТЯТЬ)"`,
        translate: 'keep - kept - kept', 
        url: kept
    },
    {
        word: `Три формы глагола "ЗНАТЬ"`,
        translate: 'know - knew - known', 
        url: know
    },
    {
        word: `Три формы глагола "УЧИТЬ (изучать)"`,
        translate: 'learn - learnt/lerned - learnt/lerned', 
        url: learn
    },
    {
        word: `Три формы глагола "ПОКИДАТЬ"`,
        translate: 'leave - left - left', 
        url: leave
    },
    {
        word: `Три формы глагола "ТЕРЯТЬ"`,
        translate: 'lose - lost - lost', 
        url: loseLost
    },
    {
        word: `Три формы глагола "ДЕЛАТЬ (создавать)"`,
        translate: 'make - made - made', 
        url: make
    },
    {
        word: `Три формы глагола "ОЗНАЧАТЬ"`,
        translate: 'mean - meant/ment - meant/ment', 
        url: meant
    },
    {
        word: `Три формы глагола "ВСТРЕТИТЬ"`,
        translate: 'meet - met - met', 
        url: meet
    },
    {
        word: `Три формы глагола "ПЛАТИТЬ"`,
        translate: 'pay - payd - payd', 
        url: pay
    },
    {
        word: `Три формы глагола "КЛАСТЬ"`,
        translate: 'put - put - put', 
        url: put
    },
    {
        word: `Три формы глагола "ЧИТАТЬ"`,
        translate: 'read - read/red - read/red', 
        url: readRead
    },
    {
        word: `Три формы глагола "ЗВЕНЕТЬ"`,
        translate: 'ring - rang - rung', 
        url: ring
    },
    {
        word: `Три формы глагола "БЕЖАТЬ"`,
        translate: 'run - ran - run', 
        url: runRan
    },
    {
        word: `Три формы глагола "СКАЗАТЬ"`,
        translate: 'say - sayd - sayd', 
        url: say
    },
    {
        word: `Три формы глагола "ВИДЕТЬ"`,
        translate: 'see - saw - seen', 
        url: seeSaw
    },
    {
        word: `Три формы глагола "ПРОДАВАТЬ"`,
        translate: 'sell - sold - sold', 
        url: sellSold
    },
    {
        word: `Три формы глагола "ПОСЫЛАТЬ"`,
        translate: 'send - sent - sent', 
        url: sendLetters
    },
    {
        word: `Три формы глагола "ПОКАЗЫВАТЬ"`,
        translate: 'show - showed - shown', 
        url: show
    },
    {
        word: `Три формы глагола "ЗАКРЫВАТЬ"`,
        translate: 'shut - shut - shut', 
        url: shut
    },
    {
        word: `Три формы глагола "ПЕТЬ"`,
        translate: 'sing - sang - sung', 
        url: singSang
    },
    {
        word: `Три формы глагола "СИДЕТЬ"`,
        translate: 'sit - sat - sat', 
        url: sit
    },
    {
        word: `Три формы глагола "СПАТЬ"`,
        translate: 'sleep - slept - slept', 
        url: sleepSlept
    },
    {
        word: `Три формы глагола "ГОВОРИТЬ"`,
        translate: 'speak - spoke - spoken', 
        url: speak
    },
    {
        word: `Три формы глагола "ТРАТИТЬ (ПРОВОДИТЬ ВРЕМЯ)"`,
        translate: 'spend - spent - spent', 
        url: spendSpent
    },
    {
        word: `Три формы глагола "СТОЯТЬ"`,
        translate: 'stand - stood - stood', 
        url: stand
    },
    {
        word: `Три формы глагола "ПЛАВАТЬ"`,
        translate: 'swim - swam - swum', 
        url: swimming
    },
    {
        word: `Три формы глагола "БРАТЬ"`,
        translate: 'take - took - taken', 
        url: take
    },
    {
        word: `Три формы глагола "ОБУЧАТЬ"`,
        translate: 'teach - tought - tought', 
        url: teachTaught
    },
    {
        word: `Три формы глагола "РАССКАЗЫВАТЬ"`,
        translate: 'tell - told - told', 
        url: tell
    },
    {
        word: `Три формы глагола "ДУМАТЬ"`,
        translate: 'think - thought - thought', 
        url: thinkThought
    },
    {
        word: `Три формы глагола "БРОСАТЬ"`,
        translate: 'throw - threw - thrown', 
        url: thrown
    },
    {
        word: `Три формы глагола "ПОНИМАТЬ"`,
        translate: 'understand - understood - understood', 
        url: understandUnderstood
    },
    {
        word: `Три формы глагола "ПРОСЫПАТЬСЯ"`,
        translate: 'wake - woke - woken', 
        url: wake
    },
    {
        word: `Три формы глагола "НОСИТЬ (ОДЕЖДУ)"`,
        translate: 'wear - wore - worn', 
        url: wearWore
    },
    {
        word: `Три формы глагола "ПОБЕЖДАТЬ"`,
        translate: 'win - won - won', 
        url: win
    },
    {
        word: `Три формы глагола "ПИСАТЬ"`,
        translate: 'write - wrote - written', 
        url: writeWrote
    },

]

export default unit17;

